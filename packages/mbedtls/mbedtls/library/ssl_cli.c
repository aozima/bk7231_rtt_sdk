...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
    end = ssl->out_msg + ssl->out_max_content_len;
#endif
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
    if( mbedtls_ssl_confirm_content_len( ssl, 0, buf - ssl->out_buf  +
                                           hostname_len + 9 ) )
        return;
#endif
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
    ret = mbedtls_ssl_confirm_content_len( ssl, 0, MBEDTLS_SSL_BUFFER_MIN + 1024 );
    if( ret )
        return( ret );
#endif
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
    ret = mbedtls_ssl_confirm_content_len( ssl, 0, pofs + 2 + ext_len +
...
static int ssl_write_client_key_exchange( mbedtls_ssl_context *ssl )
{
    ...
    if( mbedtls_ssl_ciphersuite_uses_psk( ciphersuite_info ) )
    {
...
#if defined(MBEDTLS_SSL_DYNAMIC_CONTENT_LEN)
        if( i + 2 + n > ssl->out_max_content_len )
#else
        ...
        if( ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK )
        {
            ...
            ret = mbedtls_ecdh_make_public( &ssl->handshake->ecdh_ctx, &n,
                    &ssl->out_msg[i], ssl->out_max_content_len - i,
                    ssl->conf->f_rng, ssl->conf->p_rng );
#else
            ret = mbedtls_ecdh_make_public( &ssl->handshake->ecdh_ctx, &n,
    ...
    if( ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE )
    {
        ...
        ret = mbedtls_ecjpake_write_round_two( &ssl->handshake->ecjpake_ctx,
                ssl->out_msg + i, ssl->out_max_content_len - i, &n,
                ssl->conf->f_rng, ssl->conf->p_rng );
#else
        ret = mbedtls_ecjpake_write_round_two( &ssl->handshake->ecjpake_ctx,
