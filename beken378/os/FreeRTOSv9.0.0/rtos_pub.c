...
OSStatus rtos_print_stack(beken_thread_t* thread)
{
    ...
    static uint32_t call_stack_buf[32] = {0};
    ...
    code_end_addr = (uint32_t)(&_begin_data);

    if (NULL == thread) {
        thread = rtos_get_current_thread();
    }

    os_memset(&xTaskStatus, 0x00, sizeof(xTaskStatus));
    ...
    stack_end_addr   = *(uint32_t*)(xTaskStatus.xHandle) + 2048;
    sp = *(uint32_t*)(xTaskStatus.xHandle);
    os_printf("task=%.16s,stack=[0x%x,0x%x],sp=0x%x\n", xTaskStatus.pcTaskName, stack_start_addr, stack_end_addr, sp);
...
#endif
    {
        ...
        if ((code_start_addr < pc) && (pc < code_end_addr) && (call_stack_index < sizeof(call_stack_buf) / sizeof(call_stack_buf[0]))) {
            call_stack_buf[call_stack_index] = pc;
            call_stack_index++;
        }
#if portSTACK_GROWTH < 0
        if ((0x01010101 == *(uint32_t*)(sp + 4)) && (0x02020202 == *(uint32_t*)(sp + 8)) && (0x03030303 == *(uint32_t*)(sp + 12))) break;
#else
        if ((0x03030303 == *(uint32_t*)(sp + 4)) && (0x02020202 == *(uint32_t*)(sp + 8)) && (0x01010101 == *(uint32_t*)(sp + 12))) break;
#endif
    }
    if (call_stack_index > 0)
    {
        int index;
        os_printf("addr2line.exe -e bk7231u.elf -a -f");
