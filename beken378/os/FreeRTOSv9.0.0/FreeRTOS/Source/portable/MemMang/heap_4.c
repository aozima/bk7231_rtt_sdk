...
typedef struct A_BLOCK_LINK
{
    ...
    struct A_BLOCK_LINK *pxNextUsingBlock;
    uint16_t             line;
    uint16_t             size;
    char                 funcname[16];
#endif
...
/*-----------------------------------------------------------*/
#if OSMALLOC_STATISTICAL
static BlockLink_t *pxUsing = NULL;
void prvInsertBlockIntoUsingList( BlockLink_t *pxBlockToInsert )
{
    pxBlockToInsert->pxNextUsingBlock = pxUsing;
    pxUsing = pxBlockToInsert;
}
void prvRemoveBlockFromUsingList( BlockLink_t *pxBlockToRemove )
{
    BlockLink_t *pxPrvBlock;
    BlockLink_t *pxCurBlock;

    if (pxBlockToRemove == pxUsing)
    {
        pxUsing = pxUsing->pxNextUsingBlock;
        pxBlockToRemove->pxNextUsingBlock = NULL;
        return;
    }

    for (pxPrvBlock = NULL, pxCurBlock = pxUsing;
         pxCurBlock != NULL;
         pxPrvBlock = pxCurBlock, pxCurBlock = pxCurBlock->pxNextUsingBlock)
    {
        if (pxBlockToRemove == pxCurBlock)
        {
            pxPrvBlock->pxNextUsingBlock = pxCurBlock->pxNextUsingBlock;
            pxBlockToRemove->pxNextUsingBlock = NULL;
            break;
        }
    }
...
#endif
{
	...
	pvReturn = malloc_without_lock(xWantedSize);
   	...
   	if(pvReturn) {
        ...
        pxLink->line = line;
        pxLink->size = (uint16_t)xWantedSize;
        memcpy(pxLink->funcname, call_func_name, sizeof(pxLink->funcname));
        pxLink->funcname[sizeof(pxLink->funcname) - 1] = '\0';
        prvInsertBlockIntoUsingList(pxLink);
   	}
#endif
	( void ) xTaskResumeAll();
		...
		if( ( pxLink->xBlockSize & xBlockAllocatedBit ) != 0 )
		{
			if( pxLink->pxNextFreeBlock == NULL )
			{
				...
				vTaskSuspendAll();
				{
					...
					prvRemoveBlockFromUsingList(pxLink);
					#endif
...
void *pvPortRealloc( void *pv, size_t xWantedSize )
{
	...
	if (pvReturn != NULL) {
#if OSMALLOC_STATISTICAL
		prvInsertBlockIntoUsingList((BlockLink_t *)((u8*)pvReturn - xHeapStructSize));
#endif
		if (pvReturn != pv)
...
{
#if OSMALLOC_STATISTICAL
    void *p = pvPortMalloc_cm((const char*)__FUNCTION__,__LINE__,xWantedSize, 0);
    if (3 == xWantedSize)
    {
        bk_printf("pvPortMalloc=%p\n", p);
        //rtos_print_stack(NULL);
    }
    return p;
#else
    return pvPortMalloc_cm((const char*)__FUNCTION__,__LINE__,xWantedSize, 0);
#endif
}
...
{
#if OSMALLOC_STATISTICAL
    BlockLink_t *pxIterator = ((u8*)pv - xHeapStructSize);
    if (3 == pxIterator->size)
    {
        bk_printf("vPortFree=%p\n", pv);
        //rtos_print_stack(NULL);
    }
#endif
    vPortFree_cm((const char*)__FUNCTION__,__LINE__,pv);
}

#endif

/* just list free nodes since we don't hold using nodes */
void list_free_nodes()
{
    BlockLink_t *pxIterator;
...
#if OSMALLOC_STATISTICAL
void list_using_nodes()
{
    BlockLink_t *pxIterator;

	/* Iterate through the list until a block is found that has a higher address
	than the block being inserted. */
	for( pxIterator = pxUsing; pxIterator != NULL; pxIterator = pxIterator->pxNextUsingBlock )
	{
		bk_printf("[%p-%d/%d]%s:%d\n", pxIterator, pxIterator->size, pxIterator->xBlockSize & ~xBlockAllocatedBit, pxIterator->funcname, pxIterator->line);
	}
}
#endif
