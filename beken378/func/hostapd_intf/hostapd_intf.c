...
#define SCAN_FAILED_MAX_COUNT_OF_INITIAL_MODE    20
static uint32_t scan_successive_failed_count = 0;
int scan_failed_too_many_times(void)
{
	return SCAN_FAILED_MAX_COUNT_OF_INITIAL_MODE<scan_successive_failed_count?1:0;
}
#endif
int wpa_get_scan_rst(struct prism2_hostapd_param *param, int len)
{
...
#if (CFG_SOC_NAME == SOC_BK7221U)
		scan_successive_failed_count ++;
		mmgmt_attempted_connect();
		if(SCAN_FAILED_MAX_COUNT_OF_INITIAL_MODE == scan_successive_failed_count)
		{
	...
	{
		scan_successive_failed_count = 0;
	}
#endif
...
int hapd_intf_ioctl(unsigned long arg)
{
    ...
    if (!param)
    {
        rt_kprintf("hapd_intf_ioctl param is null\r\n");
        goto ioctl_exit;
    }
