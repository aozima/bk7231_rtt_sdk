...
#ifdef CONFIG_NO_WPA_MSG
#undef wpa_msg
#define wpa_msg(ctx, level, args...) wpa_dbg(ctx, level, args)
#endif /* CONFIG_NO_WPA_MSG */
#ifdef CONFIG_NO_STDOUT_DEBUG
#undef wpa_printf
#define wpa_printf(level, args...) do { if (level > MSG_MSGDUMP) { bk_printf(args); bk_printf("\n"); } } while (0)
#endif /* CONFIG_NO_STDOUT_DEBUG */
#ifdef CONFIG_NO_HOSTAPD_LOGGER
#undef hostapd_logger
#define hostapd_logger(ctx, addr, module, level, args...) do { if (level > HOSTAPD_LEVEL_DEBUG_VERBOSE) { bk_printf(args); bk_printf("\n"); } } while (0)
#endif /* CONFIG_NO_HOSTAPD_LOGGER */
#endif
