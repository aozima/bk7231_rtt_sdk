...
#else
void bk_wlan_start_scan(SCAN_PARAM_T *inScanParam)
#endif
{
    ...
    /* add vif if needed */
    ret = rw_msg_send_add_if((u8 *)&g_sta_param_ptr->own_mac, NL80211_IFTYPE_STATION, 0, &add_if_cfm);
    if ((0 == ret) && (CO_OK == add_if_cfm.status))
    {
        vif_idx = add_if_cfm.inst_nbr;
        rwnx_set_scan_vif_idx(vif_idx);
    }
    os_printf("%s vif_idx=%d\n", __FUNCTION__, vif_idx);

    if (NULL == inScanParam)
    {
        os_memset((void *)&scan_param, 0, sizeof(scan_param));
    ...
    {
        inScanParam->vif_idx = vif_idx;

        rw_msg_send_scanu_req(inScanParam);
    }
}
...
{
    UINT32 linger_time,keep_timer;
    
    linger_time = 20;
    keep_timer = 10;

    bk_wlan_dtim_rf_ps_set_linger_time(linger_time);
    power_save_set_keep_timer_time(keep_timer);
...
void user_callback_register(void)
{
    ...
    rw_evt_set_callback(RW_EVT_STA_GOT_IP, test_rw_event_func);
