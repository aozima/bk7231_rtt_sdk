...
UINT32 mhdr_scanu_start_cfm(void *msg, SCAN_RST_UPLOAD_T *ap_list)
{
    ...
    //remove vif if needed
    if (0xff != scan_vif_idx)
    {
        os_printf("%s vif_idx=%d\n", __FUNCTION__, scan_vif_idx);
        rw_msg_send_remove_if(scan_vif_idx);
        scan_vif_idx = 0xff;
    }
...
void rwnx_set_scan_vif_idx(uint8_t vif_idx)
{
    scan_vif_idx = vif_idx;
}
