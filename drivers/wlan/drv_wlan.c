...
static rt_err_t beken_wlan_control(rt_device_t dev, int cmd, void *args)
{
    ...
    case WIFI_SCAN:
    {
        ...
        rt_memset((void *)&scan_param, 0, sizeof(scan_param));
        rt_memset(&scan_param.bssid, 0xff, ETH_ALEN);
        scan_param.vif_idx = 0xff;

        rt_strncpy((char *)scan_param.ssids[0].array, wlan->info->ssid, sizeof(scan_param.ssids[0].array));
        scan_param.ssids[0].length = rt_strlen(scan_param.ssids[0].array);
        if (scan_param.ssids[0].length > 0)
        {
            scan_param.num_ssids++;
        }
        bk_wlan_start_scan(&scan_param);
