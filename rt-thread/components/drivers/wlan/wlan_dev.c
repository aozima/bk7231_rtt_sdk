...
int rt_wlan_indicate_event_handle(struct rt_wlan_device *device, rt_wlan_event_t event, void *user_data)
{
    ...
    case WIFI_EVT_STA_CONNECT_FAILED:
        wifi_info = (struct beken_wifi_info *)device->user_data;
        if ((NULL != wifi_info) && (wifi_info->mode == ADVANCED_MODE))
        {
            wifi_info->mode = NORMAL_MODE;
            wlan_fast_connect_info_erase();
        }
        break;

#ifdef RT_USING_WLAN_AP
    case WIFI_EVT_AP_ASSOCIATED:
        break;

    case WIFI_EVT_AP_DISASSOCIATED:
        break;
