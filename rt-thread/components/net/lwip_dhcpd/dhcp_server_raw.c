                 ...
                 u8_t *opt_buf, u16_t len)
{
    ...
    if (opt != NULL)
    {
        ...
        if ((node != NULL) && (memcmp(node->chaddr, msg->chaddr, msg->hlen) == 0))
        {
            return node;
        }
    }
...
dhcp_server_recv(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *recv_addr, u16_t port)
{
    ...
    if(recv_netif != dhcp_server->netif)
    {
        rt_kprintf("dhcp_server_recv: recv_netif %c%c skip\n", recv_netif->name[0], recv_netif->name[1]);
        pbuf_free(p);
        return;
    }
